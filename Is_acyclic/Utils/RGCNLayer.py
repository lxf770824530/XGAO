...
class RGCNLayer(MessagePassing):
    ...
    def message(self, x_j, x_i, edge_type, message_scale, message_replacement):    #在这里就是做了对边的mask操作，若有mask（gate），则根据mask后的边的信息执行GNN
        ...
        b = torch.index_select(self.bias, 0, edge_type)  #self.bias shape:[6,100] 是代表每个边的type的100维向量(边有6个类别，每种类别是100维)，b就是根据每条边的type，从self.bias中挑选出对应的向量
        ...
        basis_messages = basis_messages[count, edge_type, :] + b   #将对应type的边信息截取出来并与b聚合
