...
from utils import Initial_graph_generate, Rectify, Fix_nodes_index, Fix_mask_single_false, Is_exixt_in_list, Draw_graph, Remove_tenor_from_list
...
class Node(object):
    def __init__(self, Graph):
        ...
        last_mask = torch.ones(len(self.graph.edge_attr)).bool()
        ...
        for i,m in enumerate(self.mask_list):
            ...
            if discriminate_result:
                ...
                remove_m_index.append(i)

                # self.mask_list.remove(m)

        self.mask_list = Remove_tenor_from_list(self.mask_list, remove_m_index)
...
def monte_carlo_tree_search(node):
    ...
    computation_budget = 100  # 对每一层的节点  总共搜索150次 最后算哪个节点最合适
    ...
    #while best_next_node.children != []:
    #    best_next_node = best_child(best_next_node, False)
...
def Run_MCTS():
    ...
    init_node = Node(Initial_graph_generate(args))
    ...
    while SA_STATE.is_continue():
        ...
        print('Current graph:',current_node.graph)
        print('Current temperature:',SA_STATE.temperature)
        ...
        Draw_graph(current_node.graph, i)
    ...
    prediction = GNN_MODEL(current_node.graph.x, current_node.graph.edge_index, current_node.graph.edge_attr, batch)
    print('Prediction:',prediction[0].max())
    Draw_graph(current_node.graph, 66666)
